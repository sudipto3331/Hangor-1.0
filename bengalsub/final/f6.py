...
# -----------------------------
TARGET_PRESSURE = 1050.0  # hPa (desired water pressure)
...
while time.time() - start_time < 20:
    ...
    if msg:
        ...
        error = TARGET_PRESSURE - water_pressure
        integral += error * dt
        derivative = (error - last_error) / dt if dt > 0 else 0.0
        output = Kp * error + Ki * integral + Kd * derivative

        # Convert PID output to RC PWM (1500 = neutral)
        pwm = 1500 + int(output)

        # Clamp PWM to safe range (1400-1600)
        ...
        # Debug print
        print(f"[SCALED_PRESSURE2] Water: {water_pressure:.2f} hPa | Target: {TARGET_PRESSURE:.2f} hPa | Error: {error:.2f} | PWM: {pwm}")
