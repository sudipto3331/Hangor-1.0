...
# -----------------------------
TARGET_PRESSURE = 1060.0  # hPa (relative water pressure)
print(f"[INFO] Target relative water pressure set to {TARGET_PRESSURE} hPa")
...
while time.time() - start_time < 20:
    ...
    if msg:
        if msg.get_type() == "SCALED_PRESSURE":
            last_air = msg.press_abs
            print(f"[SCALED_PRESSURE] Air: {last_air:.2f} hPa")

        elif msg.get_type() == "SCALED_PRESSURE2":
            last_water = msg.press_abs
            print(f"[SCALED_PRESSURE2] Water: {last_water:.2f} hPa")

        # Only control when both values are available
        if last_air is not None and last_water is not None:
            relative_pressure = last_water
            print(f"[DEPTH_PRESSURE] Relative: {relative_pressure:.2f} hPa (Target: {TARGET_PRESSURE:.2f} hPa)")

            if relative_pressure < TARGET_PRESSURE:
                # Too shallow → dive
                rc_override[0] = 1400
                rc_override[1] = 1400
                send_rc_override()
            ...
            elif relative_pressure > TARGET_PRESSURE:
                # Too deep → rise
                rc_override[0] = 1600
                rc_override[1] = 1600
                send_rc_override()
